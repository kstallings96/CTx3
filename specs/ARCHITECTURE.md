...
## Day 2 — MonsterMaker, a full period

MonsterMaker is a 60-minute period of its own, not a warm-up. It is the only
...
MonsterMaker has moved to its own day. Two rules of the same tier back to back,
...
recording on screen. MonsterMaker's literal half is deterministic and needs no
network at all.
...
- MonsterMaker's projector cells are class-level while its hands-on phases are
...
- ~~Is Literal Genie a whole-class projector demo or a hands-on station?~~
  ...
  **MonsterMaker** (SPEC-monstermaker.md), where the literal executor is a control
  condition rather than an activity: determinism becomes the comparison, not
  the content.
- ~~Is MonsterMaker whole-class only?~~ No. Thirty of its sixty minutes are
